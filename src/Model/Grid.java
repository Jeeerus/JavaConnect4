...
/**
 * Grid Glass: The play field of the game. Usually,
 * the grid's dimension is a 7 by 6. However, This 
 * 
 * @author Jairus and Andrew (16948980)
 */
public final class Grid implements java.io.Serializable{
    ...
    /**
     * This method fills a Grid with spaces.
     * This method needs to be called when a grid object is instantiated.
     */ 
    ...
    /**
     * Displays the contents of the character array
     * in a grid format to mimic how connect four is 
     * played in real life.
     * 
     * This should be called each time the array is updated to 
     * show any changes made to the grid.
     */
    ...
    /**
     * This method 'drops' down the 'chip' into the grid,
     * depending on the user's chosen column. In terms of
     * the program, it (method) takes in the column and finds
     * goes through the array, checks if the column is full or not,
     * and adds the character into the bottom of the array (The highest,
    ...
    /**
     * This method checks if the latest chip dropped by the user is
     * connected to 3 more chips; Vertical, Horizontal or Diagonal to it.
     * 
    ...
    public boolean checkConnectFour(int column, char chip){
        int startRow = 0;
        System.out.println("Given Col: "+ column);
    ...
    /**
     * A method that takes in the row and column of the chip that was
     * last dropped. Checks if there are 3 more of the same char under it
     * using a for loop. For every same char, 1 is added to the counter.
     * counter must reach four.
     * 
    ...
    /**
     * This method checks if the latest dropped chip causes four 
     * chips to be horizontally connected. Same concept as vertical
     * checker; using a counter.
     * 
    ...
    /**
     * Method that checks if chips connected diagonally (North East and South West)
     * Using two sets of nested for loops : One that goes up 
    ...
    /**
     * Method that checks if chips connected diagonally (North West and South East)
     * Using two sets of nested for loops : One that goes up 
     * and one that goes down.]
     * 
     * @param row - row of latest chip
    ...
    /**
     * Method that checks to see if the grid is full by checking the top 
     * row [0] in every column
